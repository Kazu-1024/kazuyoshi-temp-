...
const CreateVocabulary = () => {
  ...
  const fetchVocabulary = async () => {
    try {
      const response = await fetch("http://localhost:8080/getvocabularys", {
        credentials: "include",
      });
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/postvocabularys", fetchOptions);
        ...
        <div className="max-w-md shadow-2xl bg-white mx-[4%] p-4 border-2 border-black rounded-b-xl">
          <div className="max-h-[58dvh] overflow-y-auto">
            {vocabularyList && vocabularyList.length > 0 ? (
              ...
              <p className="text-center mt-4">まだ登録された単語はありません</p>
            )}
          </div>
